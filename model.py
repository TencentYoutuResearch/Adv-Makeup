...
class MakeupAttack(nn.Module):
    ...
    def save_res_img(self, idx, before, before_path, simi_scores_dict, target_name):
        ...
        :return: None
        """
        save_dir = self.data_dir + '/test_imgs' + '/' + target_name.split('.')[0]
    ...
    def save_res_tmp_img(self, idx, before, before_path, target_name):
        ...
        :return: None
        """
        save_dir = self.data_dir + '/test_imgs_tmp' + '/' + target_name.split('.')[0]
        os.makedirs(save_dir, exist_ok=True)
        fake_content = self.enc(before, bn_training=False)
        fake_after = self.dec(*fake_content, bn_training=False)
    ...
    def visualization(self, ep_num, batch_num):
        ...
        torchvision.utils.save_image(self.orig_before[0:4] / 2 + 0.5, './log/before.png', nrow=1)
        torchvision.utils.save_image(self.orig_after[0:4] / 2 + 0.5, './log/after.png', nrow=1)
        torchvision.utils.save_image(self.gen_img[0:4] / 2 + 0.5, './log/res.png', nrow=1)
        torchvision.utils.save_image(self.orig_before_pasted[0:4] / 2 + 0.5, './log/before_pasted.png', nrow=1)
